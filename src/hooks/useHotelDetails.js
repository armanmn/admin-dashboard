// // src/hooks/useHotelDetails.js
// "use client";
// import { useEffect, useMemo, useRef, useState } from "react";
// import api from "@/utils/api";
// import { buildChildrenParamsFromCSV } from "@/utils/childrenCsv";
...
//   nights = 1,
//   rooms = 1,
//   adults = 2,        // adults CSV (e.g. "2,2") կամ թիվ (legacy)
//   children = 0,      // children CSV (e.g. "1,2") կամ թիվ (legacy)
//   cityCode,
//   childrenAges = "", // ages CSV՝ "5,9|11" (comma=room ages, pipe=rooms)
//   filterBasis = "",
//   maxOffers = 8,
//   maximumWaitTime = 15,
// }) {
...
//       // deliberately not writing offers: [] here
//     });

//     // ✅ normalize children + ages CSV before any request
//     const { children: childrenCsv, childrenAges: agesCsv } =
//       buildChildrenParamsFromCSV(children, childrenAges);

//     console.log("[HD] QS kids", { childrenCsv, agesCsv });
...
//       children: String(childrenCsv),
//       childrenAges: String(agesCsv || ""),
...
//         const offers = Array.isArray(foundHotel?.offersPreview)
//           ? foundHotel.offersPreview.map((o) => ({
//               price: o?.price
//                 ? { ...o.price }
...
//           (Array.isArray(foundHotel?.images)
//             ? foundHotel.images[0]?.url || foundHotel.images[0]
//             : null) ||
//           null;
...
//         `/suppliers/goglobal/hotel-info?hotelId=${encodeURIComponent(hotelId)}`
//       );
//       const raw = unwrapResponse(res);
//       const node = extractHotelNode(raw);
...
//       const pictures = uniq(
//         (node?.pictures || node?.images || []).map((p) => p?.url || p?.URL || p)
//       );
//       return {
//         name: node?.name || "",
...
//   return {
//     loading,
//     hotel,            // ✅ hotel.offers when available
//     offersPreview,    // for views that use separate list
//     heroPhotos,
//     totalPhotoCount,
//     fullAddress,
//     areaLabel,
...
import { useEffect, useMemo, useRef, useState } from "react";
import api from "@/utils/api";
// ⛔️ հանում ենք buildChildrenParamsFromCSV import-ը — վերցնում ենք տեղային helpers-ներով
...
function buildAgesByRooms(childrenCSV = "", flatOrCsv = "") {
  ...
  if (!val) return "";
  if (val.includes("|")) return val; // already room-segmented
  ...
  if (flat.length !== totalKids) return ""; // don't guess if counts mismatch
...
function getSnapshotChildrenAgesCSV() {
  try {
    const snap = JSON.parse(sessionStorage.getItem("lastHotelSnapshot") || "{}");
    return snap?.criteria?.childrenAgesCSV || "";
  } catch {
    return "";
  }
}

function normalizePaxCSV(adults, children, childrenAges) {
  // adults/children may come as number or CSV string
  const adultsCSV =
    typeof adults === "string" ? adults : String(Number(adults ?? 2));
  const childrenCSV =
    typeof children === "string" ? children : String(Number(children ?? 0));

  // collect potential ages inputs (priority order)
  const agesCandidate =
    (typeof childrenAges === "string" || Array.isArray(childrenAges)) && childrenAges
      ? childrenAges
      : getSnapshotChildrenAgesCSV();

  // canonicalize ages to room-aware CSV
  let childrenAgesCSV = "";
  if (typeof agesCandidate === "string") {
    childrenAgesCSV = agesCandidate.includes("|")
      ? agesCandidate
      : buildAgesByRooms(childrenCSV, agesCandidate);
  } else if (Array.isArray(agesCandidate)) {
    // allow [[...],[...]] or flat [...]
    if (Array.isArray(agesCandidate[0])) {
      childrenAgesCSV = agesCandidate.map((r) => r.join(",")).join("|");
    } else {
      childrenAgesCSV = buildAgesByRooms(childrenCSV, agesCandidate.join(","));
    }
  }

  // fallback ONLY if we actually have kids and still no ages
  const perRoomCounts = splitCSV(childrenCSV).map((n) => Number(n) || 0);
  const totalKids = perRoomCounts.reduce((a, b) => a + b, 0);
  ...
  // shape sanity check (rooms x kids)
  const actualShape = (childrenAgesCSV ? childrenAgesCSV.split("|") : []).map((seg) =>
    seg ? seg.split(",").filter(Boolean).length : 0
  );
  const shapeOk =
    actualShape.length === perRoomCounts.length &&
    perRoomCounts.every((n, i) => n === actualShape[i]);

  if (!shapeOk) {
    // try to rebuild from any flat ages source we had; if not, final fallback
    let flat = "";
    if (typeof agesCandidate === "string") flat = splitCSV(agesCandidate).join(",");
    else if (Array.isArray(agesCandidate)) flat = agesCandidate.flat().join(",");

    const rebuilt = buildAgesByRooms(childrenCSV, flat);
    childrenAgesCSV =
      rebuilt || (totalKids > 0 ? makeDefaultAgesCSV(childrenCSV, 8) : "");
  }

  return { adultsCSV, childrenCSV, childrenAgesCSV, totalKids };
}
/* --------------------------------------------------------------------- */
...
export function useHotelDetails({
  ...
  children = 0,      // children CSV (e.g. "1,2") կամ թիվ (legacy)
  cityCode,
  childrenAges = "", // ages CSV՝ "5,9|11" (comma=room ages, pipe=rooms) կամ array
  filterBasis = "",
  maxOffers = 8,
  maximumWaitTime = 15,
}) {
  ...
  useEffect(() => {
    ...
    // ✅ adults/children/ages → canonical CSV (room-aware ages)
    const { adultsCSV, childrenCSV, childrenAgesCSV, totalKids } = normalizePaxCSV(
      adults,
      children,
      childrenAges
    );

    console.log("[HD] QS kids", { childrenCsv: childrenCSV, agesCsv: childrenAgesCSV });
    ...
    const qsAvail = new URLSearchParams({
      ...
      nights: String(nights),
      rooms: String(rooms),
      adults: String(adultsCSV),
      children: String(childrenCSV),
      ...(totalKids > 0 ? { childrenAges: String(childrenAgesCSV) } : {}),
      ...(filterBasis ? { filterBasis: String(filterBasis) } : {}),
      maxOffers: String(maxOffers),
      maximumWaitTime: String(maximumWaitTime),
    }).toString();
